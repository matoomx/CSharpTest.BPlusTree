...
{
	static IEnumerable<KeyValuePair<int, string>> MakeValues(int start, int count)
    {
    ...
    public void TestSyncFromLogging()
    {
		...
		var options = BPlusTree.CreateOptions<int, string>(new PrimitiveSerializer(), new PrimitiveSerializer());
		...
		using var copy = BPlusTree.Create(readcopy);
		using var tlog = new TransactionLog<int, string>(new TransactionLogOptions<int, string>(System.IO.Path.ChangeExtension(tempFile.TempPath, ".tlog"), PrimitiveSerializer.Int32, PrimitiveSerializer.String) { ReadOnly = true });
		tree.Add(0, "0");
		tree.Commit();

		long logpos = 0;
		//start by copying the data from tree's file into the copy instance:
		copy.BulkInsert(BPlusTree.EnumerateFile(options), new BulkInsertOptions { InputIsSorted = true, CommitOnCompletion = false, ReplaceContents = true });
