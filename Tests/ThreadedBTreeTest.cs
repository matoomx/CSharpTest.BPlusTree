...
public class ThreadedBTreeTest
{
    ...
    public void TestAtomicUpdate()
    {
        ...
        time.Start();
        using (var tree = BPlusTree.Create(PrimitiveSerializer.Int32, PrimitiveSerializer.Int32))
        {
            tree[1] = 0;
            using (var w = new WorkQueue(threads))
            {
    ...
    //int StartAndAbortWriters(BPlusTree<KeyInfo, DataValue>.Options options, TempFile copy)
    //{
    //    RecordsCreated = 0;
    ...
    public void TestErrorsOnInsertAndDelete()
    {
        ...
        var options = BPlusTree.CreateOptions(new KeyInfoSerializer(), new DataValueSerializer(), new KeyInfoComparer());
        options.CalcBTreeOrder(32, 300);
        options.FileName = TempFile.TempPath;
        options.CreateFile = CreatePolicy.Always;

		using var dictionary = BPlusTree.Create(options);
		using var work = new WorkQueue(Environment.ProcessorCount);
		Exception lastError = null;
		work.OnError += delegate (object o, ErrorEventArgs e) { lastError = e.GetException(); };
		...
		Assert.IsNull(lastError, $"Exception raised in worker: {lastError}");
	}
    ...
    class ThreadedTest
    {
        ...
        public void Run()
        {
            ...
            try
            {
                var values = new Dictionary<KeyInfo, DataValue>(new KeyInfoEquality());
                ...
                var found = new Dictionary<KeyInfo,DataValue>(values, new KeyInfoEquality());
                ...
                {
                    read++;
					Assert.IsTrue(_data.TryGetValue(kv.Key, out DataValue test));
					Assert.AreEqual(kv.Key.UID, test.Key.UID);
                    Assert.AreEqual(kv.Value.Hash, test.Hash);
                    Assert.IsTrue(kv.Value.Bytes.SequenceEqual(test.Bytes));
                }
                ...
                foreach (KeyValuePair<KeyInfo, DataValue> kv in values)
                {
                    ...
                    read++;
                    Assert.IsTrue(bytes.SequenceEqual(_data[kv.Key].Bytes));
                }
