...
public class TestTransactionLog
{
    ...
    public void TestTransactionLogOptions()
    {
		...
		temp.Delete();
		var opt = new TransactionLogOptions<int, string>(temp.TempPath, PrimitiveSerializer.Int32, PrimitiveSerializer.String);
		//FileName
		...
		using var log = new TransactionLog<int, string>(opt);
    ...
    public void TestLogWithJunkAppended()
    {
		...
		using (var io = File.OpenWrite(tmp.TempPath))
		{
    ...
    public void TestLogCorruption()
    {
		...
		var TestVariants = new Converter<KeyValuePair<int, byte[]>, byte[]>[]
		{
