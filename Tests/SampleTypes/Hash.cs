...
public sealed class Hash : Comparable<Hash>
{
    ...
    private static Hash Create<T>(byte[] data) where T : HashAlgorithm, new()
    {
		using T algo = new T();
		return new Hash(algo.ComputeHash(data == null ? Empty : data));
	}

    private static Hash Create<T>(Stream data) where T : HashAlgorithm, new()
    {
        using (data)
        using (T algo = new T())
...
#pragma warning restore SYSLIB0021 // Type or member is obsolete
    ...
    {
        return (HashAlgorithm)CryptoConfig.CreateFromName(AlgorithmName);
    }
    ...
    public string AlgorithmName
    {
        get
        {
				return _hashCode.Length switch
				{
					16 => ("MD5"),
					20 => ("SHA1"),
					32 => ("SHA256"),
    ...
    {
        return other == null ? 1 : _hashCode.SequenceEqual(other._hashCode) ? 0 : 1;
    }

    /// <summary> Returns a hash of the hash code :) </summary>
    protected override int HashCode
    {
        get { return _hashCode.GetHashCode(); }
    }
}
