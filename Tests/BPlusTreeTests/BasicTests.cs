...
public class BasicTests
{
    ...
    protected virtual BPlusTree<int, string>.Options Options
    {
        get
        {
			return BPlusTree.CreateOptions(PrimitiveSerializer.Int32, PrimitiveSerializer.String);
        }
    }

	protected virtual BPlusTree<int, string>.Options OptionsWithCount
	{
		get
		{
			var opt = BPlusTree.CreateOptions(PrimitiveSerializer.Int32, PrimitiveSerializer.String);
			opt.EnableCount = true;
			return opt;
		}
	}
    ...
    {
        var options = OptionsWithCount;
        options.BTreeOrder = 4;
		using var data = BPlusTree.Create(options);
    ...
    public void TestEnumerateFrom()
    {
		...
		Assert.HasCount(50, new List<KeyValuePair<int, string>>(data.EnumerateFrom(50)));
		Assert.HasCount(25, new List<KeyValuePair<int, string>>(data.EnumerateFrom(75)));
    ...
    {
		using var data = BPlusTree.Create(OptionsWithCount);
    ...
    public void TestBulkInsert()
    {
        ...
        {
            using (BPlusTree<int, string> data = BPlusTree.Create(OptionsWithCount))
            {
    ...
    {
		using var dataNoCount = BPlusTree.Create(Options);
		{
			Assert.AreEqual(int.MinValue, dataNoCount.Count);
		}
    ...
    public void TestInserts()
	{
		...
		int[][] TestArrays =
		[
				[10, 18, 81, 121, 76, 31, 250, 174, 24, 38, 246, 79],
				[110,191,84,218,170,217,199,232,184,254,32,90,241,136,181,28,226,69,52],
		];
    ...
    public void TestKeyValueCollections()
    {
		...
		//Key collection
		Assert.HasCount(data.Count, data.Keys);
		Assert.IsTrue(data.Keys.IsReadOnly);
		for (int i = 0; i < sample.Count && i < 5; i++)
			Assert.Contains(sample[i].Key, data.Keys);
		...
		//Value collection
		Assert.HasCount(data.Count, data.Values);
		Assert.IsTrue(data.Values.IsReadOnly);
		for (int i = 0; i < sample.Count && i < 5; i++)
			Assert.Contains(sample[i].Value, data.Values);
	...
	static void IsSameList(Dictionary<int, string> keys, BPlusTree<int, string> data)
    {
		...
		{
			count++;
			Assert.IsTrue(data.TryGetValue(key, out string test));
			Assert.AreEqual(key.ToString(), test);
		}
