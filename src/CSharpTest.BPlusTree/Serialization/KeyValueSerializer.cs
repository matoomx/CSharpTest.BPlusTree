...
public sealed class KeyValueSerializer<TKey, TValue> : ISerializer<KeyValuePair<TKey, TValue>>
{
    ...
    public void WriteTo(KeyValuePair<TKey, TValue> value, IBufferWriter<byte> stream)
    {
    ...
    public KeyValuePair<TKey, TValue> ReadFrom(ReadOnlySequence<byte> stream, ref SequencePosition position)
    {
        return new KeyValuePair<TKey, TValue>(_keySerializer.ReadFrom(stream, ref position), _valueSerializer.ReadFrom(stream, ref position));
    }
}
