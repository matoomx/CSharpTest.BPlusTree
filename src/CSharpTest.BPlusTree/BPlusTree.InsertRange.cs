...
partial class BPlusTree<TKey, TValue>
{
    ...
    private int AddRange(NodePin thisLock, ref KeyRange range, AddRangeInfo value, NodePin parent, int parentIx)
    {
        ...
        if (me.IsLeaf)
        {
            ...
            {
				bool exists = me.BinarySearch(_itemComparer, new Element(value.Current.Key), out int ordinal);
				Check.Assert<DuplicateKeyException>(!exists || value.AllowUpdate);
        ...
        {
			me.BinarySearch(_itemComparer, new Element(value.Current.Key), out int ordinal);
        ...
        try
        {
            ...
            using (RootLock root = LockRoot(LockType.Insert, false))
            {
                ...
                if (oldRoot.Ptr.Count == 0 || bulkOptions.ReplaceContents)
                {
                    ...
                    items = OrderedKeyValuePairs<TKey, TValue>.WithDuplicateHandling(items, new KeyValueComparer<TKey, TValue>(_options.KeyComparer), bulkOptions.DuplicateHandling);
                }
                else 
                {
                    // Merging with existing data and enforce duplicate keys...
                    items = OrderedKeyValuePairs<TKey, TValue>.Merge(_options.KeyComparer, bulkOptions.DuplicateHandling, EnumerateNodeContents(oldRoot), items);
                }
    ...
    private Node BulkWrite(ICollection<StorageHandle> handles, ref int counter, IEnumerable<KeyValuePair<TKey, TValue>> itemsEnum)
	{
        ...
        for (int i = 1; i < working.Count; i++)
        {
            ...
            if (me.Count < limitMin)
            {
                ...
                bool success = _storage.Storage.TryGetNode(parent[prev].ChildNode.StoreHandle, out Node prevNode, _storage.NodeSerializer);
                Check.Assert(success, "Failed to load node");
                ...
                while (me.Count < limitMin)
                {
                    ...
                    if (me.Count + 1 == limitMin)
                    {
                        ...
                        else me.Insert(0, new Element(default, item.ChildNode));
    ...
    private void InsertWorkingNode(ICollection<StorageHandle> handles, List<Node> working, int index, Element child)
    {
        if(index < 0)
        {
            ...
            if (working.Count > 1)
                working[0].Insert(0, new Element(default, new NodeHandle(working[1].StorageHandle)));
            index++;
        }
        ...
        if(parent.Count == 0)
            parent.Insert(parent.Count, new Element(default, child.ChildNode));
        else
