...
public sealed partial class BPlusTree<TKey, TValue> : IDisposable, ITransactable, IDictionary<TKey, TValue>
{
    ...
    /// </summary>
    public BPlusTree(Options ioptions)
    {
        bool fileExists =
        ...
        _storage = _options.CachePolicy switch
        {
            CachePolicy.All => new NodeCacheFull(_options),
            CachePolicy.Recent => new NodeCacheNormal(_options),
            CachePolicy.None => new NodeCacheNone(_options),
            _ => throw new InvalidConfigurationValueException("CachePolicy"),
        };
        ...
        {
            if (_options.ExistingLogAction == ExistingLogAction.Truncate || _options.ExistingLogAction == ExistingLogAction.Default && !fileExists)
            {
                _options.LogFile.TruncateLog();
            }
            else if (_options.LogFile.Size > 0 && (_options.ExistingLogAction == ExistingLogAction.Replay || _options.ExistingLogAction == ExistingLogAction.ReplayAndCommit || (_options.ExistingLogAction == ExistingLogAction.Default && fileExists)))
            {
                bool commit = (_options.ExistingLogAction == ExistingLogAction.ReplayAndCommit || (_options.ExistingLogAction == ExistingLogAction.Default && fileExists));
                bool merge = _options.StorageType == StorageType.Disk && new FileInfo(_options.FileName).Length < _options.LogFile.Size;

                if (merge) // log data is larger than we are...
                {
                    var opts = new BulkInsertOptions
                    {
                        CommitOnCompletion = commit,
                        DuplicateHandling = DuplicateHandling.LastValueWins,
                        InputIsSorted = true,
                        ReplaceContents = true
                    };

                    BulkInsert(_options.LogFile.MergeLog(_options.KeyComparer, File.Exists(ioptions.FileName) ? EnumerateFile(ioptions) : []), opts);
                }
                else
                {
	...
	public void Dispose()
    {
        ...
        try
        {
            ...
            try
            {
                if (!IsReadOnly && _options.CommitOnDispose)
                {
    ...
    void CommitChanges(bool requiresLock)
    {
        ...
        try
        { 
            if (_storage.Storage is ITransactable transactable)
                transactable.Commit();
    ...
    void OnChanged()
    {
        if (_options.TransactionLogLimit > 0 && _options.LogFile != null && _options.LogFile.Size > _options.TransactionLogLimit)
        {
    ...
    public int Count 
    {
        get { return _hasCount ? _count : int.MinValue; } 
    }

    /// <summary> Returns the lock timeout being used by this instance. </summary>
    int LockTimeout 
    { 
        get { return _options.LockTimeout; } 
    }
    ...
    public TValue this[TKey key]
    {
        ...
        set
        {
            var ii = new InsertValue(value, true);
            AddEntry(key, ref ii);
        }
    }
    ...
    {
        var ii = new InsertValue(item.Value, false);
    ...
    public int AddRange(IEnumerable<KeyValuePair<TKey, TValue>> unorderedItems, bool allowUpdates)
    {
        ...
        var items = new KeyValuePair<TKey, TValue>[2048];
        var kvcomparer = new KeyValueComparer<TKey, TValue>(_keyComparer);
    ...
    {
        int result = 0;
        using (var bulk = new AddRangeInfo(allowUpdates, items))
        {
    ...
    {
        var ii = new InsertValue(value, false);
        AddEntry(key, ref ii);
    }
    ...
    {
        var ii = new InsertionInfo(fnCreate, IgnoreUpdate);
        AddEntry(key, ref ii);
        return ii.Value;
    }
    ...
    {
        var ii = new InsertionInfo(addValue, fnUpdate);
        AddEntry(key, ref ii);
        return ii.Value;
    }
    ...
    {
        var ii = new InsertionInfo(fnCreate, fnUpdate);
        AddEntry(key, ref ii);
        return ii.Value;
    }
    ...
    /// <see cref="T:CSharpTest.Collections.Generic.ICreateOrUpdateValue`2"/> interface.
    /// </summary>
    public bool AddOrUpdate<T>(TKey key, ref T createOrUpdateValue) where T : ICreateOrUpdateValue<TKey, TValue>
    {
        var result = AddEntry(key, ref createOrUpdateValue);
    ...
    {
        var ri = new RemoveIfValue(item.Key, item.Value);
    ...
    {
        var ri = new RemoveIfPredicate(fnCondition);
        return RemoveEntry(key, ref ri) == RemoveResult.Removed;
    }

    /// <summary>
    /// Conditionally removes a key/value pair from the dictionary via an implementation of the
    /// <see cref="T:CSharpTest.Collections.Generic.IRemoveValue`2"/> interface.
    /// </summary>
    ...
    /// </summary>
    public ICollection<TKey> Keys { get { return _keysCollection ??= new KeyCollection(this); } }

    /// <summary>
    /// Gets an <see cref="T:System.Collections.Generic.ICollection`1"/> containing the values in the <see cref="T:System.Collections.Generic.IDictionary`2"/>.
    /// </summary>
    public ICollection<TValue> Values { get { return _valuesCollection ??= new ValueCollection(this); } }
