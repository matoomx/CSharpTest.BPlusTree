...
partial class BPlusTree<TKey, TValue>
{
    ...
    sealed partial class NodeCacheNormal : NodeCacheBase
    {
        ...
        private ReaderWriterLocking _cacheLock;
        ...
        protected override void LoadStorage()
        {
			...
			var rootHandle = new NodeHandle(Storage.OpenRoot(out bool isNew));
            ...
            Storage.TryGetNode(rootHandle.StoreHandle, out _root.Node, NodeSerializer);
				Check.Assert(_root.Node != null, "Unable to load storage root.");
        }
        ...
        public override void ResetCache()
        {
			...
			Check.Assert<InvalidNodeHandleException>(_root.Handle.StoreHandle.Equals(Storage.OpenRoot(out bool isnew)));
			if (isnew)
                _root.Node = CreateRoot(_root.Handle);
        }
        ...
        CacheEntry GetCache(NodeHandle handle, bool isNew)
        {
            ...
            if (!isNew)
            {
				...
				using var rx = _cacheLock.Read(base.Options.LockTimeout);

				if (_cache.TryGetValue(handle, out weakRef))
				{
                    if (!weakRef.TryGetTarget(out entry))
                    {
						using var dx = new SafeLock<DeadlockException>(weakRef);
            ...
            if (entry == null)
            {
				using var wx = _cacheLock.Write(base.Options.LockTimeout);
            ...
            Check.Assert(entry != null, "Cache entry is null");
        ...
        public override ILockStrategy CreateLock(NodeHandle handle, out object refobj)
        {
            ...
            Check.Assert(locked, "No write lock");
            refobj = entry;
            return entry.Lock;
        }
        ...
        protected override NodePin Lock(NodePin parent, LockType ltype, NodeHandle child)
        {
            ...
            Check.Assert<DeadlockException>(locked != NoLock);
            try
            {
                Node node = entry.Node;
                if (node == null)
                {
					using var dx = new SafeLock<DeadlockException>(entry);
					node = entry.Node;
					if (node == null)
					{
						Check.Assert<InvalidNodeHandleException>(
						...
						Check.Assert(null == old, "Collision on cache load.");
					}
				}
        ...
        public override void UpdateNode(NodePin node)
        {
            ...
            CacheEntry entry = node.Reference as CacheEntry ?? throw new InvalidNodeHandleException();
				
            if (node.IsDeleted)
            {
                Check.Assert(node.LockType != LockType.Read, "Lock type read was not expected");
