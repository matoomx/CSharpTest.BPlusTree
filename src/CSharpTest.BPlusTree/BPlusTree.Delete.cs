...
partial class BPlusTree<TKey, TValue>
{
    ...
    struct RemoveIfValue : IRemoveValue<TKey, TValue>
    {
        ...
        public RemoveIfValue(TKey _, TValue value)
        {
            _value = value;
        }
        bool IRemoveValue<TKey, TValue>.RemoveValue(TKey key, TValue value)
        ...
        where T : IRemoveValue<TKey, TValue>
    {
        ...
        if (me.Count <= minimumKeys && parent != null && !parent.Ptr.IsRoot)
        {
            ...
            Check.Assert(false, "Failed to join node before delete.");
        }
    ...
    private static void CopyElements(Node src, int srcIndex, Node dest, int destIndex, int count, TKey firstSrcKey)
    {
        for (int ix = 0; ix < count; ix++)
        {
            Element item = src[srcIndex + ix];
            if ((destIndex + ix) == 0 && !src.IsLeaf)
                item = new Element(default, item.ChildNode); //uses empty key at index zero on non-leaf
    ...
    private void Join(NodePin small, NodePin big, bool moveToBigger, NodePin parent, int parentSmallIndex)
    {
		...
		Check.Assert(small.Handle.Equals(parent.Ptr[parentSmallIndex].ChildNode), "Incorrect parent ordinal for left.");
		Check.Assert(big.Handle.Equals(parent.Ptr[parentBigIndex].ChildNode), "Incorrect parent ordinal for right.");
		...
		if (small.Ptr.Count + big.Ptr.Count <= fillFactor || (small.Ptr.Count + big.Ptr.Count) <= (minimumKeys * 2))
		{
			...
			Check.Assert(removeItem.IsNode && removeItem.ChildNode.Equals(big.Handle), "Invalid parent index in join.");
		...
		else //ballances the distribution of nodes between two children, using moveToBigger to resolve rounding up/down
		{
			...
			CopyElements(small.Ptr, borrowing, newBig.Ptr, newBig.Ptr.Count, small.Ptr.Count - borrowing, default);
