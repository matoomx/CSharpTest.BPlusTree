...
partial class BPlusTree<TKey, TValue>
{
	sealed class NodeTransaction : IDisposable, ITransactable
    {
        ...
        {
			Check.Assert(pin.LockType != LockType.Read, "Node is not locked for update");
        ...
        {
			Check.Assert(pin.LockType != LockType.Read, "Node is not locked for update");
			Check.Assert(_parentItem == null, "An update is already in this operation");
            _parentItem = pin;
            pin.BeginUpdate();
            return pin.Ptr;
        }
        ...
        public void Commit()
        {
			Check.Assert(_committed == false, "Transaction has already been committed.");
        ...
        public void Dispose()
        {
            try
            {
				var buffer = _logToken.Stream;
				buffer?.Dispose();
				_logToken.Stream = null;
                _logToken.OperationCount = 0;
